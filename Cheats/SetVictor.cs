...
{
    internal class SetVictor : CustomCommand
    {
        public override string Name => "Set Victor Boxes";

        public override string Description => "set how many Victor boxes you want";

        public override string Format => "/setvictor <amount>";
        ...
        public override void Execute(CommandInput message)
        {
            ...
            foreach (var box in vault.LocalLunchBoxes)
                if (box.LunchBoxType == ELunchBoxType.Victor)
                    toRemove.Add(box);
            foreach (var box in toRemove)
                vault.RemoveLunchBox(box);
            for (int i = 0; i < amount; i++)
                vault.AddLunchBox(ELunchBoxType.Victor);
            Utils.DisplayMessage("Victor boxes set to: " + amount);
            }
            catch (Exception e){
